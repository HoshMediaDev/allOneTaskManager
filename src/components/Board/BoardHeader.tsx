...
export const BoardHeader: React.FC<BoardHeaderProps> = ({ title, onCreateList }) => {
  const [showMenu, setShowMenu] = useState(false);
  const [isEditing, setIsEditing] = useState(false);
  const { currentBoard, updateBoard, deleteBoard } = useBoardStore();
  ...
  return (
    <div className="flex justify-between items-center mb-6">
      ...
      <AddListButton onClick={onCreateList} />
    </div>
  );
};