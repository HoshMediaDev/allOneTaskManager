...
export const CreateListModal: React.FC<CreateListModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!title.trim() || !currentBoard || (isPrivateBoard && !userEmail)) return;
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Create ${isPrivateBoard ? 'Private' : ''} List`}>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex justify-end space-x-3 pt-4">
          ...
          <button
            ...
            disabled={isSubmitting}
          >
            {isSubmitting ? 'Creating...' : `Create ${isPrivateBoard ? 'Private ' : ''}List`}
          </button>
        </div>
      </form>
    </Modal>
